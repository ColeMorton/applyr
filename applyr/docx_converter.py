...
class DOCXConverter:
    ...
    def _is_heading(self, para) -> bool:
        ...
        return (para.style.name.startswith('Heading') or 
                para.style.name.startswith('Title'))
    ...
    def _is_list_item(self, para) -> bool:
        ...
        return para.style.name in ['List Bullet', 'List Number', 
    ...
    def _is_likely_job_title(self, text: str) -> bool:
        ...
        role_indicators = ['Engineer', 'Developer', 'Manager', 'Director', 
                           'Specialist', 'Analyst', 'Lead', 'Senior', 'Principal',
                           'Architect', 'Consultant', 'Coordinator', 'Supervisor']
    ...
    def _is_likely_company_name(self, para) -> bool:
        ...
        return (para.runs and para.runs[0].bold and 
                not self._is_heading(para))
    
    def _is_likely_achievement_section(self, text: str) -> bool:
        """Heuristic to detect achievement sections"""
        achievement_keywords = ['achievement', 'highlight', 'accomplishment', 
                              'key result', 'deliverable', 'success']
    ...
    def _apply_smart_page_breaks(self, doc, style_template: str, html_content: str = None) -> None:
        ...
        for i, para in enumerate(paragraphs):
            ...
            if article_info:
                ...
                if current_article and article_paragraphs:
                    ...
                    self.console.print(f"[dim]   ✓ Protected article[{current_article['index']}] ({len(article_paragraphs)} paragraphs)[/dim]")
        ...
        if current_article and article_paragraphs:
            self._apply_keep_together_to_article(article_paragraphs)
            articles_protected += 1
            self.console.print(f"[dim]   ✓ Protected article[{current_article['index']}] ({len(article_paragraphs)} paragraphs, no end marker)[/dim]")
        
        self.console.print(f"[dim]✨ Applied {page_breaks_applied} page breaks and protected {articles_protected} article blocks[/dim]")
    ...
    def _should_have_page_break(self, para_text: str, class_map: dict) -> bool:
        ...
        for text_snippet in class_map.keys():
