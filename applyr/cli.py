...
app = typer.Typer(name="applyr", help="🚀 Intelligent Job Market Analysis Toolkit", rich_markup_mode="rich")
console = Console()
...
@app.command("scrape")
def scrape_command(
    url: Optional[str] = typer.Option(None, "--url", "-u", help="Single SEEK job URL to scrape"),
    urls_file: Optional[Path] = typer.Option(
        None, "--urls-file", "-f", help="File containing SEEK URLs (one per line)"
    ),
    output_dir: Path = typer.Option(
        "data/outputs/job_descriptions", "--output-dir", "-o", help="Output directory for markdown files"
    ),
...
def batch_command(
    urls_file: Path = typer.Option(
        "scripts/job_scraper/job_urls.txt", "--urls-file", "-f", help="File containing SEEK URLs (one per line)"
    ),
    output_dir: Path = typer.Option(
        "data/outputs/job_descriptions", "--output-dir", "-o", help="Output directory for markdown files"
    ),
    delay: float = typer.Option(3.0, "--delay", "-d", help="Delay between requests in seconds (respectful scraping)"),
):
...
def aggregate_command(
    input_dir: Path = typer.Option(
        "data/outputs/job_descriptions", "--input-dir", "-i", help="Input directory containing job description files"
    ),
    output_file: Optional[Path] = typer.Option(
        None, "--output-file", "-o", help="Output file path (default: auto-generated with date)"
    ),
    date: Optional[str] = typer.Option(
        None, "--date", "-d", help="Date for filename in YYYYMMDD format (default: today)"
    ),
...
@app.command("pdf")
def pdf_command(
    input_path: Path = typer.Argument(..., help="Input markdown/HTML file or directory containing markdown/HTML files"),
    output: Optional[Path] = typer.Option(
        None, "--output", "-o", help="Output PDF file or directory (default: same name as input with .pdf extension)"
    ),
    style: Optional[str] = typer.Option(
        None, "--style", "-s", help="Style name (looks for {style}.css in applyr/styles/)"
    ),
    css_string: Optional[str] = typer.Option(None, "--css", "-s", help="Inline CSS string for styling the PDF"),
    batch: bool = typer.Option(False, "--batch", "-b", help="Process all markdown and HTML files in a directory"),
    skip_lint: bool = typer.Option(False, "--skip-lint", help="Skip HTML processing and validation"),
    no_css: bool = typer.Option(False, "--no-css", help="Disable default CSS styling (forces unstyled output)"),
):
    ...
    if batch or input_path.is_dir():
        ...
        results = converter.batch_convert(input_path, output_dir, resolved_css_file, css_string, skip_lint)
        ...
        if input_path.suffix.lower() not in [".md", ".html"]:
            console.print("[red]❌ Error: Input must be a markdown (.md) or HTML (.html) file[/red]")
            raise typer.Exit(1)
        ...
        success = converter.convert_to_pdf(input_path, output_pdf, final_css_file, final_css_string, skip_lint)
...
def resume_formats_command(
    input_path: Path = typer.Argument(..., help="Input resume markdown or HTML file"),
    output_dir: Optional[Path] = typer.Option(
        None, "--output-dir", "-o", help="Output directory for all formats (default: data/outputs/pdf/)"
    ),
    formats: Optional[str] = typer.Option(
        "ats", "--formats", "-f", help="Comma-separated list of formats: ats,technical"
    ),
    skip_lint: bool = typer.Option(False, "--skip-lint", help="Skip HTML processing and validation"),
):
    ...
    Available formats:
    - sensylate: Brand-consistent design with perfect color alignment
    ...
    style_mapping = {
        ...
        "technical": Path("applyr/styles/technical.css")
        if Path("applyr/styles/technical.css").exists()
        else Path("applyr/styles/ats.css"),
    }
    ...
    for format_name in track(format_list, description="Generating formats..."):
        ...
        success = converter.convert_to_pdf(input_path, output_pdf, css_file=css_file, skip_lint=skip_lint)
...
def validate_pdf_command(
    pdf_path: Path = typer.Argument(..., help="PDF file to validate and analyze"),
    detailed: bool = typer.Option(False, "--detailed", "-d", help="Show detailed quality analysis"),
):
    ...
    if pdf_path.suffix.lower() != ".pdf":
        console.print("[red]❌ Error: File must be a PDF (.pdf)[/red]")
        raise typer.Exit(1)
    ...
    size_color = {"optimal": "green", "good": "blue", "large": "yellow", "oversized": "red", "minimal": "yellow"}.get(
        size_rating, "white"
    )
...
def format_html_command(
    input_path: Path = typer.Argument(..., help="HTML file or directory containing HTML files to format and validate"),
    output: Optional[Path] = typer.Option(
        None, "--output", "-o", help="Output file or directory (default: overwrites input)"
    ),
    skip_lint: bool = typer.Option(False, "--skip-lint", help="Skip all formatting and validation"),
    show_capabilities: bool = typer.Option(False, "--capabilities", help="Show available HTML processing capabilities"),
):
    ...
    if show_capabilities:
        ...
        cap_table.add_row("Fail-Fast Mode", "[green]✅[/green]", "Professional tools required (no fallbacks)")
        cap_table.add_row("WeasyPrint Rules", "[green]✅[/green]", "PDF generation compatibility checks")
    ...
    if input_path.is_file():
        ...
        if input_path.suffix.lower() != ".html":
            console.print("[red]❌ Error: Input must be an HTML (.html) file[/red]")
            raise typer.Exit(1)
        ...
        # Process HTML
        processed_content, changes = processor.process_html(html_content, input_path, skip_lint=skip_lint)
        ...
        for html_file in track(html_files, description="Processing HTML files..."):
            try:
                ...
                # Process HTML
                processed_content, changes = processor.process_html(html_content, html_file, skip_lint=skip_lint)
                ...
                results[html_file.name] = {"success": True, "changes": len(changes), "changes_list": changes}
...
def format_export_command(
    input_path: Path = typer.Argument(..., help="HTML file to format and export with WeasyPrint-optimized processing"),
):
    ...
    console.print("[bold blue]🔧 Processing HTML with WeasyPrint optimization[/bold blue]")
...
def status_command(
    job_filter: Optional[str] = typer.Option(
        None, "--filter", "-f", help="Filter by status: discovered, interested, applied, interviewed, rejected, closed"
    ),
...
def update_status_command(
    ...
    status: str = typer.Argument(
        ..., help="New status: discovered, interested, applied, interviewed, rejected, closed"
    ),
):
...
@app.command("cleanup")
def cleanup_command(
    days: int = typer.Option(30, "--days", "-d", help="Remove closed/rejected jobs older than this many days"),
    ...
    if not confirm:
        ...
        if not df.empty:
            ...
            old_closed_jobs = df[(df["date_discovered"] < cutoff_date) & df["status"].isin(["closed", "rejected"])]
...
@app.command("add-job")
def add_job_command(
    job_identifiers: str = typer.Argument(
        ..., help="Job ID(s) or URL(s) - SEEK 8-digit IDs, Indeed 16-char hex IDs, or full job URLs, comma-separated"
    ),
    priority: Optional[str] = typer.Option("medium", "--priority", "-p", help="Job priority: high, medium, low"),
    notes: Optional[str] = typer.Option(None, "--notes", "-n", help="Custom notes about this job"),
    force: bool = typer.Option(False, "--force", "-f", help="Skip duplicate confirmation prompt"),
):
    ...
    if invalid_identifiers:
        ...
        console.print("[yellow]   SEEK: applyr add-job 12345678[/yellow]")
        ...
        console.print(
            "[yellow]   Employment Hero: applyr add-job https://jobs.employmenthero.com/AU/job/company-position-id[/yellow]"
        )
        raise typer.Exit(1)
    ...
    if indeed_jobs_to_process:
        ...
        for indeed_job in indeed_jobs_to_process:
            ...
            if not job_data:
                ...
                console.print(
                    "[yellow]  2. Copy entire page (Cmd+A, Cmd+C on Mac / Ctrl+A, Ctrl+C on Windows)[/yellow]"
                )
                console.print(f"[yellow]  3. Save to data/raw/jobs/{raw_job_id}.txt[/yellow]")
                console.print("[yellow]  4. Run this command again[/yellow]")
...
"""
                ...
                database.add_job(
                    ...
                    else f"https://au.indeed.com/viewjob?jk={raw_job_id}",
...
"""
                ...
                database.add_job(
                    ...
                    url=identifier
                    if identifier.startswith("http")
                    else f"https://www.linkedin.com/jobs/view/{raw_job_id}/",
                    status=JobStatus.DISCOVERED,
                    priority=priority_enum,
                )
    ...
    if scraping_jobs_to_process:
        ...
        try:
            scraping_results = scrape_jobs(
                scraping_jobs_to_process, Path("data/outputs/job_descriptions"), 2.0, console, database
            )
        except Exception as e:
    ...
    try:
        ...
        for indeed_job in indeed_jobs_to_process:
            all_jobs_to_check.append(
                {"identifier": indeed_job["identifier"], "job_id": indeed_job["job_id"], "source": "indeed"}
            )
        ...
        for url in scraping_jobs_to_process:
            ...
            job_id = scraper.extract_job_id(url)
            all_jobs_to_check.append({"identifier": url, "job_id": job_id, "source": "scraping"})
...
def ats_command(
    ...
    job_description: Optional[Path] = typer.Option(
        None, "--job-desc", "-j", help="Job description file for keyword matching"
    ),
    output_format: str = typer.Option("table", "--format", "-f", help="Output format: table, json, detailed"),
    save_report: bool = typer.Option(False, "--save", "-s", help="Save detailed report to file"),
):
    ...
    try:
        ...
        # Perform analysis
        console.print("[bold blue]🔍 Performing ATS analysis...[/bold blue]")
...
def docx_command(
    ...
    output: Optional[Path] = typer.Option(
        None, "--output", "-o", help="Output DOCX file path (defaults to input filename with .docx extension)"
    ),
    style: str = typer.Option(
        "professional", "--style", "-s", help="Style template: sensylate, executive, ats, professional"
    ),
    css_file: Optional[Path] = typer.Option(None, "--css-file", "-c", help="Custom CSS file for styling"),
    skip_lint: bool = typer.Option(False, "--skip-lint", help="Skip HTML processing and validation"),
):
    ...
    try:
        ...
        success = converter.convert_to_docx(
            input_file=input_path, output_docx=output, style_template=style, css_file=css_file, skip_lint=skip_lint
        )
        ...
        if success:
            ...
            if style == "ats":
                console.print("[yellow]💡 ATS-optimized formatting applied for maximum compatibility[/yellow]")
        else:
            console.print("[red]❌ DOCX conversion failed[/red]")
            raise typer.Exit(1)
...
def txt_command(
    ...
    try:
        ...
        if not parsed_data:
            console.print("[red]❌ Failed to parse document[/red]")
            console.print("[yellow]💡 Possible causes:[/yellow]")
            console.print(
                "[yellow]   • Missing required libraries (BeautifulSoup4, PyPDF2, pdfplumber, python-docx)[/yellow]"
            )
        ...
        if not raw_text or not raw_text.strip():
            console.print("[yellow]⚠️  No text content extracted from document[/yellow]")
        ...
        console.print("[green]✅ Text extraction completed[/green]")
