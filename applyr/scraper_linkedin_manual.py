    ...
    def parse_job_data(self, text_content: str) -> Dict[str, str]:
        ...
        try:
            ...
            for i, line in enumerate(lines[:20]):
                ...
                if re.match(r'^[A-Z][a-z]+,\s*[A-Z][a-z]+', line):  # City, State
                    location_candidates.append(line)
                elif re.match(r'^[A-Z][a-z]+,\s*[A-Z][a-z]+,\s*[A-Z][a-z]+', line):  # City, State, Country
                    location_candidates.append(line)
                elif re.match(r'^[A-Z][a-z]+\s+[A-Z][a-z]+$', line) and len(line.split()) == 2:  # City State
                    location_candidates.append(line)
            ...
            job_keywords = [
                ...
                'job description', 'about the job', 'about the role'
            ]
            ...
            if description_start is not None:
                ...
                for line in lines[description_start:]:
                    ...
                    if any(keyword in line.lower() for keyword in ['report job', 'save job', 'apply now', 'linkedin may', 'show more', 'show less']):
                        break
