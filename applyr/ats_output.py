...
class ATSOutputFormatter:
    ...
    def _display_overall_score(self, result):
        ...
        self.console.print(Panel(
            panel_content,
            title="[bold]ATS Compatibility Score[/bold]",
            border_style=color,
            padding=(1, 2)
        ))
    ...
    def _display_recommendations(self, recommendations: List[str]):
        ...
        format_recs = [r for r in recommendations if any(word in r.lower() for word in ['format', 'structure', 'html', 'pdf'])]
        keyword_recs = [r for r in recommendations if any(word in r.lower() for word in ['keyword', 'skill'])]
        content_recs = [r for r in recommendations if any(word in r.lower() for word in ['content', 'achievement', 'verb'])]
        other_recs = [r for r in recommendations if not any(r in ' '.join(contact_recs + format_recs + keyword_recs + content_recs) for r in [r])]
