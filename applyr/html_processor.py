...
class HTMLProcessor:
    ...
    def process_html(
        ...
        if not self.prettier_available:
            raise RuntimeError(
                "Prettier is required for HTML formatting but not available. "
        ...
        processed_content, prettier_changes = self._format_with_prettier(processed_content, source_path, weasyprint_context)
        changes_made.extend(prettier_changes)
        
        # Phase 2: Linting and auto-fixing with html-eslint
        processed_content, eslint_changes = self._lint_with_html_eslint(processed_content, source_path, weasyprint_context)
    ...
    def _format_with_prettier(self, html_content: str, source_path: Optional[Path], weasyprint_context: bool = False) -> Tuple[str, List[str]]:
        ...
        try:
            ...
            result = subprocess.run(
                ...
                text=True,
                timeout=30,
                cwd=Path.cwd()  # Ensure we're in the project directory
            )
...
{fragment_content}
    ...
    def _lint_with_html_eslint(self, html_content: str, source_path: Optional[Path], weasyprint_context: bool = False) -> Tuple[str, List[str]]:
        ...
        try:
            ...
            if result.stderr:
                ...
                if remaining_issues:
                    self.console.print(f"[yellow]⚠️  Remaining HTML issues after auto-fix:[/yellow]")
    ...
    def _validate_weasyprint_compatibility(self, html_content: str) -> List[str]:
        ...
        unsupported_properties = [
            'transform-origin', 'animation', 'transition', 
