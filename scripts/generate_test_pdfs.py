...
def main():
    ...
    for template_name in css_templates:
        ...
        if not css_file.exists():
            ...
            results.append(
                {"template": template_name, "status": "CSS not found", "output": str(output_pdf), "file_size": "N/A"}
            )
            continue
        ...
        # Convert to PDF
        success = pdf_converter.convert_markdown_to_pdf(resume_md, output_pdf, css_file=css_file)
        ...
        if success:
            ...
            results.append(
                {"template": template_name, "status": "❌ Failed", "output": str(output_pdf), "file_size": "N/A"}
            )
    ...
    for result in results:
        table.add_row(result["template"], result["status"], result["file_size"], result["output"])
    ...
    console.print("2. Verify brand text is horizontally centered in the header")
