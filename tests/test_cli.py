...
class TestAddJobCommand:
    ...
    def test_add_job_seek_id(self, runner, temp_dir, mock_responses, sample_seek_html):
        """Test adding job with SEEK 8-digit ID"""
        job_id = "87066700"
        mock_responses.add(
            responses.GET,
            f"https://www.seek.com.au/job/{job_id}",
            body=sample_seek_html,
            status=200
        )
    ...
    def test_add_job_with_priority_flag(self, runner, temp_dir, mock_responses, sample_seek_html):
    ...
    def test_add_job_mixed_batch(self, runner, temp_dir, mock_responses, sample_seek_html, sample_eh_html):
        ...
        mock_responses.add(
            responses.GET,
            f"https://www.seek.com.au/job/{seek_id}",
            body=sample_seek_html,
            status=200
        )
        mock_responses.add(
    ...
    def test_add_job_multiple_seek_ids(self, runner, temp_dir, mock_responses, sample_seek_html):
        ...
        mock_responses.add(
            responses.GET,
            f"https://www.seek.com.au/job/{job_id1}",
            body=sample_seek_html,
            status=200
        )
        mock_responses.add(
            responses.GET,
            f"https://www.seek.com.au/job/{job_id2}",
            body=sample_seek_html,
            status=200
        )
...
class TestAddJobCommandErrorScenarios:
    ...
    def test_add_job_network_failure(self, runner, temp_dir, mock_responses):
    ...
    def test_add_job_malformed_comma_list(self, runner):
        ...
        result = runner.invoke(app, ["add-job", "87066700,,87066701"])
        
        # Should handle empty items in list
        # May succeed by skipping empty items or fail with error

    def test_add_job_with_force_flag(self, runner, temp_dir, mock_responses, sample_seek_html):
...
class TestAddJobCommandValidation:
    ...
    def test_validate_seek_id_exactly_8_digits(self, runner):
        ...
        # 7 digits
        result = runner.invoke(app, ["add-job", "8706670"])
        assert result.exit_code == 1
        
        # 9 digits
        result = runner.invoke(app, ["add-job", "870667000"])
        assert result.exit_code == 1
...
class TestAddJobCommandIntegration:
    ...
    def test_job_id_format_employment_hero(self, runner, temp_dir, mock_responses, sample_eh_html):
